...
export async function fetchRepoFile({ token, repo, path, ref }) {
  ...
  if (!response.ok) {
    throw createGitHubError(`GitHub fetch failed: ${response.status}`, response.status);
  }
...
export async function writeRepoFile({ token, repo, path, branch, content, sha, message }) {
  ...
  if (!response.ok) {
    throw createGitHubError(`GitHub write failed: ${response.status}`, response.status);
  }
...
export async function listRepoDirectory({ token, repo, path, ref }) {
  ...
  if (!response.ok) {
    throw createGitHubError(`GitHub directory listing failed: ${response.status}`, response.status);
  }
